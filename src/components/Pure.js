...
class Asteroids extends React.Component {
  constructor(props) {
    ...
    this.menu = ['-- menu --'].concat(
      _.get(props, 'menu', ['reset', 'restart'])
    )
    this.menu_option = 1
    this.shooting = false
    ...
    this.ROID_PTS_LGE = _.get(props, 'gameVariables.asteroids.points.large', 20)
    ...
    this.SHIP_BLINK_DUR = _.get(props, 'gameVariables.ship.blink_duration', 0.1)
    ...
    this.SHOW_BOUNDING = _.get(props, 'gameVariables.ship.show_bounding', false)
    this.SHOW_CENTRE_DOT = _.get(props, 'gameVariables.ship.show_centre', false)
  ...
  drawMenu = () => {
    ...
    for (let i = 0; i < this.menu.length; i++) {
      ...
      const menuPosition =
        ...
        ((this.TEXT_SIZE * 0.75) / 2) * this.menu.length
      const sectionPosition =
        i * (((this.TEXT_SIZE * 0.75) / 2) * this.menu.length)
      const selected = i === this.menu_option
      ctx.fillText(
        (selected ? '> ' + value : value).toUpperCase(),
        this.canvas.width / 2,
        menuPosition + sectionPosition
      )
    }
  }
  ...
  newGame = (reset = false) => {
    ...
    if (reset) {
      this.scoreHigh = 0
      localStorage.setItem(this.SAVE_KEY_SCORE, 0)
    }
    this.newLevel()
  }
